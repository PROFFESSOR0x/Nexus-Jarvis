...
var CAM_DIST0 = 10.6;
var RING_SP = [0.14, -0.11]; // formation ring speeds (rigid, so slots never collide)
function angDiff(a, b) { return Math.atan2(Math.sin(a - b), Math.cos(a - b)); }
/* slot formation: actives hold evenly-spaced seats on 2 counter-rotating
   rings (same speed per ring = spacing is forever constant); retired park
   outside on slow private orbits. Recomputed on every membership change. */
function layoutOrbits() {
  var act = [], prk = [];
  liveWorkers().forEach(function (w) { (w.retired ? prk : act).push(w); });
  var per = [[], []];
  act.forEach(function (w, i) { per[i % 2].push(w); });
  for (var r = 0; r < 2; r++) {
    for (var k = 0; k < per[r].length; k++) {
      var w = per[r][k];
      w._slot = { ring: r,
        base: (k / per[r].length) * Math.PI * 2 + r * 0.7,
        r: 3.4 + r * 0.95,
        yAmp: 0.85 + ((k + r) % 3) * 0.28,
        tiltF: 0.9 };
      if (w._orbA == null) w._orbA = w._slot.base + (S.ringPh ? S.ringPh[r] : 0);
    }
  }
  prk.forEach(function (w) {
    w._slot = null;
    var h = hashStr(w.id);
    w._park = { r: 5.2 + (h % 3) * 0.4, y: -0.6 + ((h >> 3) % 5) * 0.45,
      sp: (((h >> 5) % 2) ? -1 : 1) * (0.05 + ((h >> 6) % 3) * 0.02) };
    if (w._orbA == null) w._orbA = (h % 628) / 100;
  });
}
/* advance one worker angle: chase the moving seat (actives) or drift (parked) */
function stepWorkerAngle(w, dt, chase) {
  if (w.retired) {
    if (!w._park) layoutOrbits();
    w._orbA += dt * (w._park ? w._park.sp : 0.05);
    return;
  }
  if (!w._slot) layoutOrbits();
  if (!w._slot) return;
  var want = w._slot.base + S.ringPh[w._slot.ring];
  w._orbA += angDiff(want, w._orbA) * Math.min(1, dt * chase);
}
function hashStr(s) {
...
var S = {
  ...
  leader: null,            // entity record (see makeEntity)
  ...
  hud: { mode: "IDLE", tools: 0, workers: 0, round: "\u2014", model: "\u2014", mem: 0 },
  ...
  ringPh: [0, 0],      // formation ring phases (advanced in the loops)
...
var E = {
  ...
  ensureWorker: function (id, label, tint) {
    if (!id || id === "leader") return S.leader;
    var w = S.workers[id];
    if (w) {
  ...
  /* violet command pulse leader -> worker */
  commandPulse: function (id, n) {
    S.workT = Math.min(1.3, (S.workT || 0) + 0.35);
    if (S.engine === "three") T3.lightPing("leader", "#b48cff", 25);
    var k = Math.max(1, Math.min(4, n || 1));
    for (var i = 0; i < k; i++) {
      S.pulses.push({ a: "leader", b: id, t: -Math.random() * 0.35,
  ...
  resultPulse: function (id, ok) {
    S.pulses.push({ a: id, b: "leader", t: 0, sp: 0.030,
  ...
  thinkingPulse: function (who) {
    ...
    var w = getEntity(who);
    if (w) { w.think = 1; w.lastAction = "thinking\u2026"; }
    E.ping(0.2);
  },
  ...
  planPulse: function (who) {
    ...
    S.pulses.push({ a: who, b: "leader", t: 0, sp: 0.028, color: "#b48cff", arch: 0 });
    trimPulses();
    E.ping(0.15);
  },
  ...
  finalBurst: function (ok) {
    addRing("leader", ok === false ? "#ff5d5d" : GOLD.hot, 0.7);
    if (S.leader) {
      S.leader.flare = Math.min(1.2, (S.leader.flare || 0) + 0.55);
      S.leader.spinKick = Math.min(1.2, S.leader.spinKick + 0.3);
    }
    if (S.engine === "three") T3.lightPing("leader", GOLD.hot, 22);
    E.ping(0.25);
  },
...
function boot() {
  ...
  S.leader = makeEntity("leader", "NEXUS", "leader", GOLD.mid);
  makeLabel("leader", "NEXUS", GOLD.mid);
...
T3.init = function (THREE) {
  ...
  /* hologram table: polar grid + floor light */
  try {
    var polar = new THREE.PolarGridHelper(6.5, 12, 5, 72, 0x0f5a5e, 0x0a3a3e);
    polar.position.y = -2.05;
    polar.material.transparent = true;
    polar.material.opacity = 0.16;
    T3.scene.add(polar);
    T3.grid = polar;
  } catch (e) { /* older three without PolarGridHelper */ }
...
T3.buildEntityNode = function (w, R, emberN, moteN) {
  ...
  node.coreMat = new THREE.MeshBasicMaterial({ color: new THREE.Color(GOLD.core) });
  ...
  node.atmoMat = new THREE.MeshBasicMaterial({ color: new THREE.Color(GOLD.mid),
  ...
  group.add(node.atmo);
  node.glow = T3.texSprite(TEX.glow, GOLD.mid, R * 2.9, 0.85);
  group.add(node.glow);
  ...
  node.shellMat = T3.goldWire(GOLD.mid, 0.20);
  node.shell = new THREE.Mesh(new THREE.IcosahedronGeometry(R, 2), node.shellMat);
  group.add(node.shell);
  var outer = new THREE.Mesh(
    new THREE.IcosahedronGeometry(R * 1.14, 1), T3.goldWire(GOLD.dim, 0.10));
  ...
  /* worker accent ring (tiny colored orbit = identity, body stays gold) */
  if (w.kind === "worker") {
    var acc = new THREE.Mesh(
      new THREE.TorusGeometry(R * 1.18, Math.max(0.005, R * 0.014), 8, 90),
      new THREE.MeshBasicMaterial({ color: new THREE.Color(w.tint),
  ...
  for (var ai = 0; ai < arcDefs.length; ai++) {
    ...
    var arc = new THREE.Mesh(
      ...
      T3.goldMetal(ai === 0 ? GOLD.hot : GOLD.mid, ai === 0 ? 1.2 : 0.85));
    ...
    var tip = T3.texSprite(TEX.glow, GOLD.hot, R * 0.26, 0.95);
    group.add(tip);
  ...
  /* ambient arc riders: sparks permanently sliding on the broken rings */
  node.riders = [];
  ...
  for (var fi = 0; fi < nFil; fi++) {
    ...
    var tube = new THREE.Mesh(
      ...
      new THREE.MeshBasicMaterial({ color: new THREE.Color(fi % 2 ? GOLD.copper : GOLD.hot),
  ...
  (function dial() {
    ...
    var cDim = new THREE.Color(GOLD.dim);
  ...
  (function dust() {
    ...
    var cA = hexToRgb(GOLD.dim), cB = hexToRgb(GOLD.deep), cC = hexToRgb(GOLD.copper);
    for (var k = 0; k < N; k++) {
  ...
  (function embers() {
    ...
    var cHot = hexToRgb(GOLD.hot), cMid = hexToRgb(GOLD.mid),
        cDeep = hexToRgb(GOLD.deep), cCop = hexToRgb(GOLD.copper);
  ...
  node.aura = T3.texSprite(TEX.glow, GOLD.mid, R * 4.4, 0.14);
  group.add(node.aura);
  /* corona disc: a plate of light beneath the sovereign */
  if (isLeader) {
    node.corona = new THREE.Mesh(
      new THREE.RingGeometry(R * 1.7, R * 2.6, 96),
      T3.holoMat(GOLD.mid, 0.10));
    node.corona.rotation.x = -Math.PI / 2;
    node.corona.position.y = -R * 1.15;
    group.add(node.corona);
  }
  ...
  node.anchor.position.set(0, R * 1.6, 0);
...
T3.spawnWorkerNode = function (w) {
  ...
  T3.buildEntityNode(w, WORKER_R, 180, 16);
...
T3.animEntity = function (w, dt, t, boost) {
  ...
  /* drive: every orbit/ring/vein accelerates while real work runs.
     S.work is raised by tool/radio/delegate events, decays on its own. */
  ...
  if (w.deadAt && !w.retired) {
    ...
    if (sAlive <= 0) { T3.removeWorkerNode(w.id); return; }
  ...
  if (w.kind === "worker") {
    /* formation seat (actives, evenly spaced, rigid) or parking drift (retired) */
    stepWorkerAngle(w, dt, (0.8 + S.work * 2.4) * pace);
    var lp = (S.leader && S.leader.node) ? S.leader.node.group.position : { x: 0, y: 0.15, z: 0 };
    var parked = w.retired && w._park;
    var srr = parked ? w._park.r : (w._slot ? w._slot.r : 3.6);
  ...
  var pulse = 1 + Math.sin(wt * (2.2 + excite * 2.5 + w.think * 4 + S.work * 1.2)) * (0.04 + excite * 0.04 + w.think * 0.05)
    + (w.flare || 0) * 0.30;
  n.core.scale.set(pulse, pulse, pulse);
  var errMix = (S.mode === "ERROR") ? 0.75 : 0;
  n.coreMat.color.set(GOLD.hot);
  if (errMix) n.coreMat.color.lerp(new T3.THREE.Color("#ff3b3b"), errMix * (0.6 + 0.4 * Math.sin(t * 14)));
  n.glow.material.opacity = clamp(0.65 + excite * 0.3 + w.think * 0.25 + w.flash * 0.3 + (w.flare || 0) * 0.4, 0, 1) * dimF;
  var gs = R * (2.9 + Math.sin(wt * 3) * 0.12 + excite * 0.35 + (w.flare || 0) * 0.6);
  n.glow.scale.set(gs, gs, 1);
  n.inner.material.opacity = 0.75 + w.think * 0.25;
  /* molten kernel flicker + breathing atmosphere */
  if (n.kernel) {
    var kp = 1 + Math.sin(wt * 7.3 + w.phase) * 0.12 + Math.sin(wt * 13.7) * 0.06 + (w.flare || 0) * 0.25;
    n.kernel.scale.set(kp, kp, kp);
  }
  if (n.atmo) {
    n.atmoMat.opacity = clamp(0.30 + excite * 0.18 + (w.flare || 0) * 0.3 + w.think * 0.12, 0, 0.9) * dimF;
    n.atmo.rotation.y -= dt * 0.3 * drive;
  }

  /* ember heat follows excitement (dynamic focus: calm bronze -> white hot) */
  if (n.emberMat) {
    n.emberMat.color.copy(T3.calmCol).lerp(T3.hotCol, clamp(0.25 + excite * 0.75, 0, 1));
  }
  ...
  if (n.emberMat) {
      ...
      n.emberMat.size = R * 0.068 * (1 + S.work * 0.45);
    }
  /* ambient arc riders sliding on the broken rings */
  if (n.riders) {
  ...
  if (n.freeTips) {
    for (var fti = 0; fti < n.freeTips.length; fti++) {
      ...
      ft.a += dt * ft.w * (0.6 + excite * 0.5 + S.work * 1.5) * (w.retired ? 0.4 : 1);
      ...
      var fs3 = R * (0.24 + 0.06 * Math.sin(wt * 7 + fti * 2.1));
  ...
  if (n.motePts && n.moteDat) {
    ...
    for (var m = 0; m < n.moteDat.length; m++) {
      var d = n.moteDat[m];
      d.y += dt * d.sp * (1 + excite + S.work * 1.6) * pace;
      d.a += dt * 0.5 * drive;
  ...
  if (n.aura) {
    n.aura.material.opacity = (0.13 + S.work * 0.14 + (w.flare || 0) * 0.25 + excite * 0.06) * dimF;
    var as = R * (4.3 + Math.sin(wt * 1.3 + w.phase) * 0.25 + S.work * 0.7);
    n.aura.scale.set(as, as, 1);
  }
  /* corona disc turns slowly, brightens with work */
  if (n.corona) {
    n.corona.rotation.z += dt * (0.1 + S.work * 0.4) * (w.retired ? 0.3 : 1);
    n.corona.material.opacity = (0.08 + S.work * 0.08 + excite * 0.04) * dimF;
  }
  /* halo tint while a halo-tool runs */
  if (w.halo) {
    n.halo.material.color.set(w.halo.color);
    n.halo.material.opacity = 0.28 + 0.12 * Math.sin(wt * 5);
  } else {
  ...
  if (n.scan) {
    w._scanT = (w._scanT || 0) + dt * (1 + S.work * 0.8) * pace;
  ...
  if (n.beam.visible) {
    ...
    n.beam.material.opacity = 0.5 * Math.max(0, 1 - bt / bttl) * (0.7 + 0.3 * Math.sin(wt * 9));
    n.beam.rotation.y += dt * 2 * (1 + S.work * 2) * pace;
  ...
  /* tool satellites: glyph sprites orbiting the shell */
  ...
  for (var oi = 0; oi < want; oi++) {
    ...
    orb.angle += dt * orb.speed * (1 + excite + S.work * 2);
    var orad = R * 1.32;
    osp.position.set(Math.cos(orb.angle) * orad, Math.sin(orb.angle * 1.3) * orad * 0.7, Math.sin(orb.angle) * orad);
    if (!osp.userData.g || osp.userData.g !== orb.glyph + orb.color) {
      var nt = new T3.THREE.CanvasTexture(glyphTex(orb.glyph, orb.color));
      osp.material.map = nt;
      osp.material.color.set(orb.color);
      osp.material.needsUpdate = true;
      osp.userData.g = orb.glyph + orb.color;
    }
    var s = R * (0.5 + 0.12 * Math.sin(wt * 6 + oi * 2));
    osp.scale.set(s, s, 1);
  }
};
...
T3.updateTethers = function () {
  ...
  Object.keys(T3.tetherLines).forEach(function (id) {
    ...
    line.material.opacity = (0.36 + S.energy * 0.28 + 0.07 * Math.sin(T3.time * 3 + id.length)) * tdim;
    var tips = T3.tetherTips[id];
    if (tips) {
      tips[0].position.set(lp.x, lp.y, lp.z);
      tips[1].position.set(wp.x, wp.y, wp.z);
      var ts = 0.42 + S.energy * 0.22 + 0.08 * Math.sin(T3.time * 5);
  ...
  if (S.pulses.length < 24) {
    ...
    for (var hi = 0; hi < liveHB.length; hi++) {
      var hw = liveHB[hi];
      if (!hw.retired && (!hw._lastBeat || T3.time - hw._lastBeat > 2.8)) {
        hw._lastBeat = T3.time;
        S.pulses.push({ a: "leader", b: hw.id, t: 0, sp: 0.022, color: "#D4AF37", arch: 0 });
        trimPulses();
      }
    }
  }
...
T3.loop = function () {
  ...
  S.ringPh[0] += dt * RING_SP[0] * (1 + S.work * 3);
  S.ringPh[1] += dt * RING_SP[1] * (1 + S.work * 3);
  var t = T3.time;
  T3.drainQueues();
  smoothChannels(dt);
  ...
  /* camera: cinematic drift + parallax + drag orbit + shake + energy push-in.
     Focus never leaves the leader: look-target tracks his live position. */
  ...
  var cd = T3.camDist - S.camE * 0.9; // slow push-in while excited, never pumping
  var ch = Math.cos(th) * cd;
  T3.camera.position.set(Math.sin(az) * ch + shx, 0.1 + Math.sin(th) * cd + shy, Math.cos(az) * ch);
  if (S.leader && S.leader.node) {
    T3.look.lerp(S.leader.node.group.position, 1 - Math.pow(0.005, dt));
  }
...
T3.updateHover = function () {
  if (!T3.raycaster) {
    try { T3.raycaster = new T3.THREE.Raycaster(); } catch (e) { return; }
  }
  var hits = [];
  if (S.leader && S.leader.node && S.leader.node.hit) hits.push(S.leader.node.hit);
  for (var i = 0; i < S.order.length; i++) {
    var w = S.workers[S.order[i]];
    if (w && w.node && w.node.hit && !w.deadAt) hits.push(w.node.hit);
  }
  if (!hits.length) { setHover(null); return; }
  T3.raycaster.setFromCamera({ x: T3.ndcX, y: T3.ndcY }, T3.camera);
  var found = null;
  try {
    var xs = T3.raycaster.intersectObjects(hits, false);
    if (xs && xs.length) found = xs[0].object.userData.eid;
  } catch (e) { /* ignore */ }
...
T3.bindInput = function () {
  ...
  window.addEventListener("pointermove", function (e) {
    ...
    T3.dragAz = clamp(T3.dragAz + (e.clientX - T3.lastPX) * 0.005, -3.14, 3.14);
    T3.dragEl = clamp(T3.dragEl - (e.clientY - T3.lastPY) * 0.005, -2.2, 2.2);
  ...
  cv.addEventListener("wheel", function (e) {
    e.preventDefault();
    T3.camDist = clamp(T3.camDist + (e.deltaY > 0 ? 0.5 : -0.5), 4, 20);
  }, { passive: false });
};
...
FL.toScreen = function (id) {
  var r = cv.getBoundingClientRect(), wrapR = wrap.getBoundingClientRect();
  var p, dy;
  if (id === "leader") { p = FL.leaderXY(); dy = -84; }
  else {
    var w = S.workers[id];
    if (!w || !w.node) return null;
    p = [w.node.x, w.node.y]; dy = -54;
  }
  return { x: (p[0] / FL.W) * r.width + (r.left - wrapR.left),
           y: (p[1] / FL.H) * r.height + (r.top - wrapR.top) + dy,
           behind: false };
};
FL.orb = function (x, y, R, hot, alpha, flick) {
  var g = FL.g;
  var gr = g.createRadialGradient(x, y, 1, x, y, R);
  gr.addColorStop(0, "rgba(255,248,225," + alpha + ")");
  gr.addColorStop(0.35, "rgba(232,196,110," + (alpha * 0.75) + ")");
  gr.addColorStop(0.7, "rgba(190,130,40," + (alpha * 0.30 * flick) + ")");
  gr.addColorStop(1, "rgba(150,100,25,0)");
  g.fillStyle = gr;
...
FL.loop = function () {
  ...
  S.ringPh[0] += 0.016 * RING_SP[0] * (1 + S.work * 3);
  S.ringPh[1] += 0.016 * RING_SP[1] * (1 + S.work * 3);
  var t = FL.t;
  ...
  live.forEach(function (w) {
    if (!w.node) return;
    stepWorkerAngle(w, 0.016, (0.8 + S.work * 2.4) * (w.retired ? 0.25 : 1));
    var parked2 = w.retired && w._park;
    var rr3 = parked2 ? w._park.r : (w._slot ? w._slot.r : 3.6);
    var yy3 = parked2 ? 0.5 : (w._slot ? w._slot.yAmp : 1.0);
    var tf3 = parked2 ? 0.9 : (w._slot ? w._slot.tiltF : 0.9);
    var yb3 = parked2 ? (L[1] - 40 + w._park.y * 20) : L[1];
    var tx2 = L[0] + Math.cos(w._orbA || 0) * rr3 * 46 + Math.sin(wt * 0.43 + w.phase) * 7;
    var ty2 = yb3 + Math.sin((w._orbA || 0) * tf3) * yy3 * 30 + Math.sin(wt * 0.9 + w.phase) * 6;
    w.node.x = lerp(w.node.x, tx2, 0.06);
    w.node.y = lerp(w.node.y, ty2, 0.06);
    var fade = w.retired ? 0.5 : 1;
    g.strokeStyle = "rgba(212,175,55," + (0.42 * fade) + ")";
    g.lineWidth = 1.6;
    g.beginPath(); g.moveTo(L[0], L[1]);
    var mx = (L[0] + w.node.x) / 2, my = (L[1] + w.node.y) / 2 - 10;
    g.quadraticCurveTo(mx, my, w.node.x, w.node.y);
    g.stroke();
    /* endpoint glows */
    g.fillStyle = "rgba(255,233,168," + (0.85 * fade) + ")";
    g.beginPath(); g.arc(L[0], L[1], 3, 0, 7); g.fill();
  ...
  for (i = S.pulses.length - 1; i >= 0; i--) {
    ...
    if (p.t < 0) continue;
    var A = (p.a === "leader") ? { x: L[0], y: L[1] } : ((S.workers[p.a] || {}).node || { x: L[0], y: L[1] });
    var B = (p.b === "leader") ? { x: L[0], y: L[1] } : ((S.workers[p.b] || {}).node || { x: L[0], y: L[1] });
  ...
  var R0 = 66 + excite * 12 + S.work * 6 + Math.sin(wt * 2.2) * 3;
  if (S.mode === "ERROR") {
    FL.orb(L[0], L[1], R0, true, 1, flick);
  ...
  live.forEach(function (w) {
    if (!w.node) return;
    var fade2 = w.retired ? 0.5 : 1;
    FL.orb(w.node.x, w.node.y, 32, false, fade2, flick);
  ...
  if (w.orbiters && w.orbiters.length) {
    tools = '<div class="tools">' + w.orbiters.map(function (o) {
      return '<span class="e3-chip" style="border-color:' + escHtml(o.color) + ";color:" + escHtml(o.color) +
        '">' + escHtml(o.glyph) + " " + escHtml(o.tool) + "</span>";
    }).join("") + "</div>";
  } else {
...
function updateLabels() {
  ...
  var ids = ["leader"].concat(S.order.filter(function (id) { return S.workers[id]; }));
  for (var i = 0; i < ids.length; i++) {
    (function (id) {
      var w = getEntity(id);
      if (!w) return;
      ...
      rec.box.style.transform = "translate(" + Math.round(pos.x + (id === "leader" ? -20 : 26)) +
        "px," + Math.round(pos.y) + "px)";
      var isHot = (S.hover.id === id);
      ...
      var sub = (w.lastAction || "").slice(0, 30);
      if (id === "leader" && !sub) sub = "MODEL " + String(S.hud.model).slice(0, 16);
      if (rec._sub !== sub) { rec.sub.textContent = sub; rec._sub = sub; }
      var chips = "";
      if (id === "leader") {
        chips = '<span class="e3-chip">MODEL ' + escHtml(String(S.hud.model).split("/").pop().slice(0, 14)) +
          '</span><span class="e3-chip">MEM ' + escHtml(String(S.hud.mem)) + ' facts</span>';
      } else {
        chips = w.orbiters.map(function (o) {
          return '<span class="e3-chip" style="border-color:' + escHtml(o.color) +
            ';color:' + escHtml(o.color) + '">' + escHtml(o.glyph) + "</span>";
        }).join("");
      }
      if (rec._chips !== chips) { rec.chips.innerHTML = chips; rec._chips = chips; }
    })(ids[i]);
  }
}
...
function updateHudDom() {
  ...
  var str = S.hud.mode + "|" + S.hud.tools + "|" + S.hud.workers + "|" +
    S.hud.round + "|" + S.hud.model + "|" + S.hud.mem;
  ...
  hudEl.innerHTML =
    ...
    escHtml(String(S.hud.workers)) + " \u00b7 " + escHtml(String(S.hud.round)) + "</span>";
}
